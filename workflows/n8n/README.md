...
- **트리거 (Trigger)**: Manual Trigger (수동 실행). 정해진 시각에 자동 반복하려면 아래 **Sync Kiwoom Portfolio (Scheduled)** 워크플로를 쓴다.
- **호출 API**: `POST http://api:8000/internal/jobs`
...
## Sync Kiwoom Portfolio (Scheduled) (`portfolio/sync-kiwoom-portfolio-scheduled.json`)

위 수동 워크플로와 **같은 작업**을 정해진 시각에 **자동으로** 큐에 넣는다. 스냅샷은 동기화할 때만 생기므로, 이 워크플로를 켜야 대시보드 **자산 추이 차트**에 하루 한 점씩 데이터가 쌓인다.

- **목적 (Purpose)**: 매일 자동으로 `SYNC_KIWOOM_PORTFOLIO` 작업을 생성해 `portfolio_snapshots`를 축적한다 → 자산 추이 차트가 채워진다.
- **차트 점 규칙**: 동기화 1회 = 스냅샷 1개지만 **차트는 하루 1점**(그날의 마지막 동기화 값)이다. 평일은 06:30·16:10 두 번 돌지만 같은 날이므로 그날 점은 16:10 값 하나다. 점이 동기화 횟수만큼 늘지 않는 건 정상이다.
- **트리거 (Trigger)**: Schedule Trigger 노드 1개에 크론 2개 (둘 다 KST):
  - `10 16 * * 1-5` — **평일 16:10**, 국내장 마감 후 종가 반영
  - `30 6 * * *` — **매일 06:30**, 미국장 마감 후 반영 (서머타임 양쪽을 커버하려고 매일로 둔다)
- **호출 API**: 수동 워크플로와 동일 — `POST http://api:8000/internal/jobs`
  - header: `X-Internal-API-Key: ={{ $env.INTERNAL_API_KEY }}`
  - body: `{ "job_type": "SYNC_KIWOOM_PORTFOLIO", "payload": {} }`
- **안전 (Read-only)**: 이 작업은 키움 **조회 TR만** 호출한다(계좌·잔고·보유종목·예수금 읽기). **주문(매수/매도)·이체는 구현되어 있지 않다** — 스케줄을 켜도 계좌에서 거래가 발생하지 않는다. 다만 켜는 순간부터 실제 증권사 API를 주기 호출하므로, 그래서 자동 활성화를 막아 둔 것이다.
- **시간대 (Timezone)**: compose에 `GENERIC_TIMEZONE=Asia/Seoul`이 있고, 워크플로 `settings.timezone`에도 `Asia/Seoul`을 명시해 두었다. 따라서 위 크론은 **KST 기준**으로 동작한다.
- **⚠️ 자동으로 켜지지 않는다**: JSON에 `"active": false`로 두었다. import만으로는 돌지 않으며 **n8n UI에서 직접 Active 토글을 켜야** 스케줄이 시작된다. (사용자가 모르는 사이에 외부 API를 주기 호출하지 않게 하려는 의도다.)
- **실패 처리 (Failure handling)**: 수동 워크플로와 동일하다(헤더 불일치 → 401, 키움 키 미설정 → worker가 `validate_configuration`에서 FAILED). 스케줄 실행 이력과 실패는 n8n **Executions** 화면에서 확인한다. 동기화 작업은 멱등(idempotent)이라 — 같은 연결에 PENDING/RUNNING 작업이 있으면 새로 만들지 않고 재사용한다 — 트리거가 겹쳐도 안전하다.

### 활성화 방법

1. 아래 **Import 방법**으로 워크플로를 등록한다.
2. http://localhost:5678 접속 → `Sync Kiwoom Portfolio (Scheduled)` 워크플로를 연다.
3. 우측 상단 **Active** 토글을 켠다. 이후 위 두 시각에 자동 실행된다.
4. 중단하려면 같은 토글을 내린다.

먼저 수동 워크플로나 대시보드의 [키움 계좌 동기화] 버튼으로 동기화가 실제로 성공하는지(키움 키가 유효한지) 확인한 뒤 켜는 것을 권장한다.
...
3. 등록할 파일 선택:
   ...
   - `workflows/n8n/portfolio/sync-kiwoom-portfolio-scheduled.json`
...
**import은 활성화가 아니다.** `Sync Kiwoom Portfolio (Scheduled)`는 `"active": false`라서 import해도 스케줄이 돌지 않는다. n8n UI에서 **Active** 토글을 직접 켜야 한다 (위 [활성화 방법](#활성화-방법) 참고).
